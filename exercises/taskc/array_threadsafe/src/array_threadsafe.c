...
/// @param mutex the mutex to protect the array
...
size_t array_find_first_private(array_mutex_t* array, const void* element,
	size_t start_pos);
...
int array_increase_capacity(array_mutex_t* array) {
	assert(array);
...
int array_decrease_capacity(array_mutex_t* array) {
	assert(array);
