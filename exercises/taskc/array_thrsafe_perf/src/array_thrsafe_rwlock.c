...
/// @param rwlock the rwlock to protect the array
typedef struct array {
	void** elements;
	size_t capacity;
	size_t count;
	pthread_rwlock_t rwlock;
} array_rwlock_rwlock_t;

// Private declarations
size_t array_rwlock_find_first_private(array_rwlock_t* array, const void* element,
	size_t start_pos);


array_rwlock_t* array_rwlock_create(size_t capacity) {
	assert(capacity);
	array_rwlock_rwlock_t* array = calloc(1, sizeof(array_rwlock_t));
	if ( array == NULL )
		return NULL;
