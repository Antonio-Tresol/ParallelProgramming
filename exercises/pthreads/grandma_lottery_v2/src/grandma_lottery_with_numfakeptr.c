...
int main(void) {
    ...
    if (error_1 == EXIT_SUCCESS && error_2 == EXIT_SUCCESS) {
        ...
        // there are memory leakeages because we cannot free the memory that was
        // allocated in procedure random_number as we return the number as a
        // fake memory address thus leaving us without the memory address we
...
void* random_number(void* seed) {
    int* random_number_produced = (int*) malloc(sizeof(int));
