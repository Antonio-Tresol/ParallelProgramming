 ...
 * @return int 
 */
int join_threads(size_t count, thread_memory_t* mem);
...
thread_memory_t* create_threads(size_t count, void*(*subroutine)(void*),
  void* data) {
  // allocate memory for the threads
  simulation_t* simulation = (simulation_t*) data;
  thread_memory_t* mem = (thread_memory_t*) calloc(count,
    sizeof(thread_memory_t));
  // allocate memory for the threads
  pthread_t* threads = (pthread_t*) calloc(count, sizeof(pthread_t));
  if (threads) {  // if calloc succeeded
    for (size_t index = 0; index < count; ++index) {
      mem[index].simulation = simulation;
      mem[index].thread_id = index;
      mem[index].number_of_threads = count;
      if (pthread_create(&threads[index], /*attr*/ NULL, subroutine,
        &mem[index]) == EXIT_SUCCESS) {
...
int join_threads(size_t count, thread_memory_t* mem) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    error += pthread_join(mem[index].self, /*value_ptr*/ NULL);
  }
  free(mem);
  return error;
}
...
int create_consumers_producers(simulation_t* simulation) {
  ...
  thread_memory_t* producers = create_threads(simulation->producer_count,
    produce, simulation);
  thread_memory_t* consumers = create_threads(simulation->consumer_count,
    consume, simulation);
