...
coordinate_t find_maximum_local_perimeter(const size_t top_left_row,
																					const size_t top_left_column,
																					const fence_data_t* fence_data);
...
int main()
{
	...
	if ( read_terrain(fence_data) ) return 2;
...
int read_terrain(fence_data_t* fence_data) {
	...
	if ( fence_data->terrain == NULL ) return 2;
...
void print_maximum_perimeter(const fence_data_t* fence_data) {
	...
	fence_data->maximum_bottom_right.row + 1,
	fence_data->maximum_bottom_right.column + 1);
}
