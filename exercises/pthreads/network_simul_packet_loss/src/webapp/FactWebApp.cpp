...
bool FactWebApp::serveHomepage(HttpRequest& httpRequest
  ...
  // TODO: change input tag
  // TODO: change the name of methods
...
bool FactWebApp::serveFactorization(HttpRequest& httpRequest
  ...
  // TODO (read )
  std::smatch matches;
  std::regex inQuery("^/fact(/|\\?number=)(\\d+)$"); //get number from query
  if (std::regex_search(httpRequest.getURI(), matches, inQuery)) {
    assert(matches.length() >= 3);
    const int64_t number = std::stoll(matches[2]);

    // TODO(you)(second part of the project): Factorization must not be done
    // by factorization threads
    // this packing operations has to be done by the packing thread.
    // Factorization must be done by the factorization threads.
    // Build the body of the response
