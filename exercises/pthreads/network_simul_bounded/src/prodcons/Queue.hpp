...
class Queue {
 ...
 protected:
  ...
  std::mutex mutex;
  /// can produce if not full, can consume if not empty
 ...
 public:
  ...
  Queue(): canProduce(1)
          ,canConsume(0) {}
  /// set the number of elements that can be produced
  ...
  void showBufferValue() {
    std::cout << "Buffer value: " << this->canProduce.getValue() << std::endl;
  }
  /// Destructor
  ~Queue() {
    // TODO(jhc): clear()?
