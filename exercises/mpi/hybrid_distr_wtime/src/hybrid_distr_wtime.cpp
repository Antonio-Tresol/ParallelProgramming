...
int main(int argc, char* argv[]) {
  ...
  if (MPI_Init(&argc, &argv) == MPI_SUCCESS) {
    ...
    if (argc == 3) {
      ...
      if (processNumber == 0) {
        ...
        for (int process = 1; process < processCount; ++process) {
          ...
          != MPI_SUCCESS) {
            std::cerr << "Error sending start value to process "
            << process << std::endl;
          }
          if (MPI_Send(&overallFinish, 1, MPI_INT, process, 0, MPI_COMM_WORLD)
           != MPI_SUCCESS) {
            std::cerr << "Error sending finish value to process "
            << process << std::endl;
          }
        }
      } else {
        if (MPI_Recv(&overallStart, 1, MPI_INT, 0, 0,
    ...
    // Declare a parallel region using OpenMP
    #pragma omp parallel default(none) \
      ...
      #pragma omp critical(stdout)  // only one thread can write to stdout
