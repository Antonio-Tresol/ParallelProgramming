...
void SieveOfEratosthenes(bool* sieve, int64_t limit) {
    memset(sieve, true, (limit + 1) * sizeof(bool));
...
void Atkin_sieve(bool *sieve, int64_t limit) {
  ...
  int64_t wlimit = (int64_t)sqrtf((long double)limit);
  int64_t i;
  int64_t j;
  int64_t x;
  int64_t y;
...
void compute_goldbach(shared_mem_t* mem, int64_t thread_count, int64_t begin,
  int64_t end) {
  ...
  default(none) shared(Array, primes, sieve, max, begin, end)
