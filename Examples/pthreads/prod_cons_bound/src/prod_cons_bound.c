...
/**
 * @brief  A struct with a bounded buffer
 * @details  The buffer has capacity `capacity` and is shared between a producer
 * and a consumer. The producer produces items and puts them in the buffer. The
 * consumer consumes items from the buffer. 
 *  it has a semaphore to count the number of items in the buffer
 */
typedef struct {
  size_t thread_count;
...
/**
 * @brief  method to analyze the arguments
 * 
 * @param argc 
 * @param argv 
 * @param shared_data 
 * @return int int 0 if everything went well, otherwise an error code
 *
 */
int analyze_arguments(int argc, char* argv[], shared_data_t* shared_data);
/**
 * @brief  method to create the threads
 * 
 * @param shared_data of a team of threads
 * @return int 0 if everything went well, otherwise an error code
 *
 */
int create_threads(shared_data_t* shared_data);
/**
 * @brief  method to produce items using the prod consumer model
 * 
 * @param data 
 * @return void* 
 *
...
/**
 * @brief  method to consume items using the prod consumer model
 * 
...
void* consume(void* data) {
  ...
  for (size_t round = 0; round < shared_data->rounds; ++round) { //we consume the number of rounds
    for (size_t index = 0; index < shared_data->buffer_capacity; ++index) { //we consume the number of elements in the buffer
      ...
      printf("\tConsumed %lg\n", value); //we print the element consumed
      // signal(can_produce)
      sem_post(&shared_data->can_produce); //we signal to the producer that they can produce
    }
  }
...
/**
 * @brief returns a random number between min and max define in seconds
 * 
 * @param min 
 * @param max 
 * @return useconds_t 
