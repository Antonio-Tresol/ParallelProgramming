...
//init cannot be thread safe because it is called before the threads are created
//it means only the main thread can call it
...
//destroy cannot be thread safe because because it destroys the mutex
//it means only the main thread can call it
...
/**
 * @brief removes the first node from a queue, but this method is not 
 * thread-safe
 * @param queue itself
 */
void queue_remove_first_unsafe(queue_t* queue) {
