 ...
 * @param process_hostname the hostname
 */
void process_values(int process_number, const char* process_hostname) {
  std::vector<double> values;
  size_t value_count = 0;
  // if we are the process 0 we read the values from the standard input
  if (process_number == 0) {
    double value = 0.0;
    while (std::cin >> value) {
  ...
  // we send the number of values to the other processes using broadcast
  // first we send the count of the values to the other processes
  // here all the processes are also receiving the value count from the process 0
  if (MPI_Bcast(&value_count, /*count*/ 1, MPI_UINT64_T, /*root*/ 0
    , MPI_COMM_WORLD) != MPI_SUCCESS ) {
    fail("could not broadcast value count");
  }
  // so they can prepare the space for the values
  values.resize(value_count);
  // then we send the values to the other processes
  // here we receive the values from the process 0 too
