...
/**
 * @brief Generates lucky statistics for the given process.
 *  it does a all reduce to get the lucky number and then
 * it compares all lucky numbers to find the lucky number
 * that is the min, the max and the average. it also reports from each process
 * the characteristics of its lucky number.
 * 
...
void generate_lucky_statistics(int process_number, int process_count) {
  ...
  // MPI has a all reduce function that does  reduce and broadcast in one step
  // it receives the value to reduce, the place to put the result(output), the
  // number of elements to reduce, the type of the elements, the operation to
  // perform, and the communicator (the world or team of processes)
